...
class Tkinter_View():
	...
	def __init__(self):
		...
		self.text_error = Text(self.root, height=1, padx = 10, pady = 10)
		self.text_roll  = Text(self.root, width=10, height=1, padx = 10, pady = 10)
		self.text_final_score = Text(self.root, width=10, height=1, padx = 10, pady = 10)
		self.text_bet = Text(self.root, width=10, height=1, padx = 10, pady = 10)
		...
		for j in range(len(columns)+1):
			...
			for i in range(len(rows)+1):
				...
				if j != 0 or i != 0:
					...
					if i == 0:
						text = Text(self.root, width=10, height=1, padx = 10, pady = 10)
						...
						self.text_bonus_and_total[(f"{columns[j-1]}", f"{rows[i-1]}")] = Text(self.root, width=10, height=1, padx = 10, pady = 10, bg="green")
						...
						self.grid_buttons[(f"{columns[j-1]}", f"{rows[i-1]}")] = Radiobutton(self.root, variable=self.box_variable, text="", value=f"{columns[j-1]}_{rows[i-1]}", indicatoron = 0, activebackground="yellow", width=10, height=1, padx = 10, pady = 10)
						self.grid_buttons[(f"{columns[j-1]}", f"{rows[i-1]}")].grid(row = i,column = j)


		text = Text(self.root, height=1, padx = 10, pady = 10, bg="blue")
		text.tag_configure("tag_name", justify='center')
		...
		for j in range(5):
			button = Checkbutton(self.root, text=1, variable=self.dice_variable[j], onvalue=f"{j}", offvalue="", indicatoron = 0, width=10, height=3, padx = 10, pady = 10)
		...
		for i in range(len(action_list)):
			...
			button = Button(self.root, text=action_list[i], width=10, height=1, padx=10, pady=10, command = partial(self.play, action_list[i].lower()))
